...
def trade_levels(st, m):
    """Calculate analysis-only entry zone, invalidation stop and targets from recent tape volatility.
    These are heuristic levels, not guaranteed execution prices or profit forecasts.
    """
    price = st.last_price
    ...
    # Recent range + average absolute price movement as a simple volatility proxy.
    ...
    # Small pullback zone around the current market price.
    ...
    return {"entry_low": max(entry_low, 0), "entry_high": max(entry_high, 0),
            "sl": max(sl, 0), "tp1": max(tp1, 0), "tp2": max(tp2, 0), "tp3": max(tp3, 0),
            "risk": risk}
...
def signal_text(st,m):
    ...
    base = (f"*{st.symbol} — {m['signal']} {m['score']}/100*\n\n"
            ...
            f"Trades: `{st.trade_count}` | Depth: `{st.depth_count}`")
    if not levels:
        return base + "\n\n⏳ *Уровни ещё рассчитываются*"
    return base + ("\n\n🎯 *РАСЧЁТНЫЕ УРОВНИ*"
                    ...
                    f"\nTP3: `{fmt_price(levels['tp3'])}`"
                    "\n\n⚠️ Уровни рассчитаны по текущей волатильности и стакану/потоку; это не гарантия движения цены.")
...
async def start(update:Update,context):
    ...
    await update.message.reply_text("🤖 Bybit Scanner V6\n\n/top — топ\n/coin BTCUSDT — монета\n/status — диагностика\n/settings — настройки")
...
async def top_cmd(update,context):
    ...
    for _,st,m,levels in rows[:10]:
        ...
        parts.append(
            ...
            f"💰 TP1 `{fmt_price(levels['tp1'])}` (R:R 1:{rr1:g})\n"
            f"💰 TP2 `{fmt_price(levels['tp2'])}` (R:R 1:{rr2:g})\n"
            f"💰 TP3 `{fmt_price(levels['tp3'])}` (R:R 1:{rr3:g})\n"
        )
...
async def settings_cmd(update,context): await update.message.reply_text(f"⚙️ V6\nSymbols: {len(SYMBOLS)}\nDepth: {DEPTH}\nWindow: {WINDOW_SEC}s\nThreshold: {SIGNAL_THRESHOLD}\nREST: DISABLED\nAPI keys: NOT REQUIRED")
