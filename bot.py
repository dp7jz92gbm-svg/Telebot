...
async def top_cmd(update,context):
    ...
    for _,st,m in rows[:10]:
        ...
        if levels:
            ...
            parts.append(f"*{st.symbol}* — {direction} `{m['score']}/100`\n"
                         ...
                         f"SL `{fmt_price(levels['sl'])}` | TP1 `{fmt_price(levels['tp1'])}` | TP2 `{fmt_price(levels['tp2'])}`\n")
        else:
            parts.append(f"*{st.symbol}* — {m['signal']} `{m['score']}/100`\nЦена `{fmt_price(st.last_price)}` | Δ `{m['delta']:.4g}` | OB `{m['imbalance']*100:+.1f}%`\n⏳ Уровни рассчитываются\n")
    text="\n".join(parts)
...
async def settings_cmd(update,context): await update.message.reply_text(f"⚙️ V5\nSymbols: {len(SYMBOLS)}\nDepth: {DEPTH}\nWindow: {WINDOW_SEC}s\nThreshold: {SIGNAL_THRESHOLD}\nREST: DISABLED\nAPI keys: NOT REQUIRED")
