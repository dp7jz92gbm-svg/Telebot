...
SUB_BATCH_SYMBOLS = int(os.getenv("SUB_BATCH_SYMBOLS", "10"))
...
# Bybit linear public orderbook supports 1/50/200/1000; 50 is a good default for this scanner.
...
def metrics(st: SymbolState) -> dict:
    ...
    # Continuous score, deliberately capped. This is a market-flow heuristic, not a prediction.
    score = 50.0
...
def handle_trade(data) -> None:
    ...
    for t in data:
        ...
        st.trade_count += 1
        last_topic = f"publicTrade.{symbol}"


def handle_orderbook(msg: dict) -> None:
    global last_topic, last_ws_error
    data = msg.get("data")
    ...
    if msg_type == "snapshot":
        ...
        if not st.book_ready:
            # We should receive a snapshot first. Ignore deltas until then.
            return
    ...
    st.depth_count += 1
    st.last_depth = now()
    last_topic = f"orderbook.{DEPTH}.{symbol}"
...
def handle_message(msg: dict) -> None:
    ...
    # Bybit may return command responses containing successTopics/failTopics.
    if msg.get("op") == "subscribe":
        data = msg.get("data") if isinstance(msg.get("data"), dict) else {}
        success_topics = data.get("successTopics") or []
        fail_topics = data.get("failTopics") or []
        if success_topics:
            subscription_ok_topics.update(success_topics)
        if fail_topics:
        ...
        elif not success_topics and not fail_topics and msg.get("success") is True:
            # The generic linear response acknowledges the whole request.
            req_topics = pending_request_topics.get(msg.get("req_id"), [])
            subscription_ok_topics.update(req_topics)
        last_subscribe_reply = "OK" if not fail_topics and msg.get("success", True) else str(fail_topics)
        return
    ...
    if topic.startswith("publicTrade."):
        ...
        # Unknown public topic is ignored rather than treated as a fatal error.
        last_topic = topic
    ...
    while True:
        try:
            ...
            async with websockets.connect(
                ...
                # No ticker stream: ticker was not needed for the scanner and was the source of
                # the observed handler error. Price is taken directly from publicTrade.
                topics = []
                for symbol in SYMBOLS:
                    topics.extend([f"publicTrade.{symbol}", f"orderbook.{DEPTH}.{symbol}"])

                for start in range(0, len(SYMBOLS), SUB_BATCH_SYMBOLS):
                    batch_symbols = SYMBOLS[start:start + SUB_BATCH_SYMBOLS]
                    batch_topics = []
                    for symbol in batch_symbols:
                        batch_topics.extend([f"publicTrade.{symbol}", f"orderbook.{DEPTH}.{symbol}"])
                    req_id = f"scan-{int(time.time() * 1000)}-{start}"
                    pending_request_topics[req_id] = batch_topics
                    await ws.send(json.dumps({"req_id": req_id, "op": "subscribe", "args": batch_topics}))
...
async def status_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    current = now()
    active_trade = sum(1 for st in states.values() if current - st.last_trade < 10)
    active_depth = sum(1 for st in states.values() if current - st.last_depth < 10)
    ...
    await update.message.reply_text(
        ...
        f"Trade active: `{active_trade}`\n"
        f"Depth active: `{active_depth}`\n"
        f"Orderbooks ready: `{books}`\n\n"
